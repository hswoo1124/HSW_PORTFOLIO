...
public class Ai : MonsterStatus
{
    ...
    void Die()
    {
        ...
        int random = Random.Range(0, 3);    //스킬잼 드랍확률 33프로 (테스트용)
        
        if(random == 0)
        {
            int rSkillJam = Random.Range(0, 5); //테스트용
            GameObject skillJamInstance = Instantiate(skillJamDropList[rSkillJam], this.gameObject.transform.position, Quaternion.identity);
        }
    ...
    private IEnumerator Attacking()
    {
        ...
        if (attacked == true)
        {
            ...
            while (true)
            {
                ...
                if (animationTime >= 0.1 && dev_Name.Contains("Slime"))
                {
                    EnemyPos(dir);

                    AttackingToMoving();
                }
                else if (animationTime >= 0.6f && type != "Archer")
                {
                    EnemyPos(dir);
    ...
    private void FixedUpdate()
    {
        ...
        dir = attackTarget.transform.position - transform.position; // 타겟 위치에 따라 시선 확인
